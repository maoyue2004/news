...
| Claude Routine ID | `trig_0197JM4tvDtBQvjTX3NqHZko` |
...
Codex 使用 `CODEX_DAILY.md`，在本地项目模式运行 Scheduled Task。它先把每日数据提交并
推送 GitHub，再通过 `.openai/hosting.json` 绑定的既有 Sites 项目保存并部署新版本。
Sites 不会因为 GitHub 有新提交而自动更新，发布步骤不能省略。

必须遵守：

- Claude Routine 与 Codex Scheduled Task 只能启用一个，避免同一时间重复抓取和竞争写入。
- 使用本地项目模式，不使用隔离 worktree；`seen.json` 和日期文件必须持续留在主分支。
- 本机需保持开机，ChatGPT 桌面应用需保持运行，并允许任务访问网络和写入项目。
- 发布时复用 `.openai/hosting.json` 的项目与 D1 绑定，绝不创建同名新 Site。
- 先测试、构建、提交并推送 GitHub，再发布同一份已提交源码。

## Claude Routine 的四条配置约束
...
Codex Scheduled Task 的完整提示词与故障保护见 `CODEX_DAILY.md`。
