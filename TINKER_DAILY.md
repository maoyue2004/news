...
- `summaryZh` — 3-6 句，**250-450 字**。说清这篇干了什么、结论是什么、有什么具体数字。
  不写「本文介绍了……」这种空话。
  （这个区间改过两次，两次都是拿实测分布去对齐实践：创刊写的是 80-160，
  2026-08-13 改成 150-260，而 2026-08-20 量创刊以来 122 条的真实分布——
  中位数 302、P10 146、P90 424、最长 497，**落在 150-260 区间里的只有 28%**，
  也就是说改过之后仍然没对齐，REVIEW 里为此连记了四次。
  这次按 P10/P90 取整定成 250-450，不再骑墙：
  下限 250 是因为不到这个字数装不下「干了什么 + 结论 + 数字」三件事，
  上限 450 是因为再长读者就不看摘要直接跳原文了。）
