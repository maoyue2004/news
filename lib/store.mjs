...
/**
 * 「这一轮没排上队」的待定账本。见 lib/tinker/defer.mjs 的说明：
 * 被名额（单源配额 / 入围上限 / thin 名额）挡掉的条目不记 seen，
 * 记在这里攒轮次，攒够才落成结论。体积很小（只有 id 和轮次），可以进 git。
 */
export function loadDeferred(dataDir) {
  return readJson(join(dataDir, 'deferred.json'), {});
}

export function saveDeferred(dataDir, deferred) {
  writeJson(join(dataDir, 'deferred.json'), deferred);
}
