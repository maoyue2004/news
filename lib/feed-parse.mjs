...
function pickRssLink(item) {
  const declared = text(item.link);
  if (declared) return decodeEntities(declared);

  // 部分正规播客 feed（例如 Megaphone）完全不提供 item <link>，只给
  // enclosure 音频地址和一个非 URL 的 GUID。没有这个回退，feed 虽然
  // 能解析出新节目，collect 阶段却会因为 link 为空把它们全部丢掉。
  const enclosure = item.enclosure;
  const audio = enclosure && typeof enclosure === 'object' ? String(enclosure['@url'] ?? '') : '';
  if (audio) return decodeEntities(audio);
