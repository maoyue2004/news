 ...
 * @param {Set}      opts.gatedIds  其中被名额挡掉的（`rejected[].gated === true`）
 * @param {object}   opts.deferred  上一轮的待定账本 `{ id: { first, attempts } }`
 * @param {string}   opts.today     `YYYY-MM-DD`
 * @param {number}   [opts.rounds]  攒够几轮落成结论
 * @returns {{ seenIds: string[], deferred: object, promoted: string[] }}
 *   `seenIds` 要写进 seen；`deferred` 是新的待定账本；
 *   `promoted` 是这一轮从待定转成结论的（攒满轮次），只为汇报用。
 */
export function planSeen({ ids, gatedIds, deferred = {}, today, rounds = DEFER_ROUNDS }) {
  ...
  for (const id of ids) {
    ...
    const attempts = (deferred[id]?.attempts ?? 0) + 1;
    if (attempts >= rounds) {
      seenIds.push(id);
      promoted.push(id);
      continue;
    }
    next[id] = { first: deferred[id]?.first ?? today, attempts };
  }
