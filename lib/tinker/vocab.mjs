...
export const TOOLS = [
  ...
  // MiniClaw：同一系具身硬件分支里 ZeroClaw 的兄弟。2026-09-07 周更体检加，
  // 理由和 ZeroClaw 那条一字不差，只是这次**不是补旧账而是当场量到的**：
  // 1377 条语料里命中 6 条、**6 条全在标题**、跨 2 个源，
  // 全部是罗西的思考的《【OpenClaw具身硬件】MiniClaw 阅读笔记（1)-(4)》系列——
  // 而 `nanoclaw` / `zeroclaw` 早就在表里，只有它不在，
  // 于是页面上点「OpenClaw」筛出来的又会混进第三个东西。
  // 三问照 ZeroClaw 走：连写复合词、不是别人的名字、没有别的名字包住它。
  // **不给带空格的 `mini claw`**——`zero claw` 那次就是撞「zero claw machine」被删的，
  // 而 mini claw machine 是同一批抓娃娃机；语料里带空格的写法 0 条，删掉零成本。
  // **也不收语料里那两处 `MimiClaw`**（作者笔误）：两条都和 MiniClaw 出现在同一篇里，
  // 加它一条标签都多认不出来——按「量到多少才动」，笔误不进表。
  { id: 'miniclaw', name: 'MiniClaw', aka: [] },
...
export const TOPICS = [
  ...
  // `子代理` 是 2026-09-07 周更体检补的，属于「别名的第三种错法」（限定词/写法收少了，
  // 而且完全不报警）：这个词条从创刊起收了 `子智能体` 和 `子 agent`，唯独漏了
  // 中文圈同样常用的第三种译法。1377 条语料里「子代理」出现 15 条，
  // **其中 10 条一个 subagent 标签都挂不上**（另外 5 条是靠同篇里的 `Subagent`
  // 英文写法蒙对的），跨 7 个源：山外云「把闭源同类产品当作子代理来调」、
  // 犀利豆「主代理和子代理复用同一套 loop」、掘金「Hermes 子代理委派」、
  // vocus「六、子代理：讀檔跟搜尋派出去」、icodex「独立 VM 子代理」。
  // 逐条看完 15 条**全部是 agent 语境，0 条是别的意思**。
  //
  // 三问里前两问干净（不是常用词的一段、不是别人的名字），**第三问命中了**：
  // 「子代理商」（分销层级里的下级代理商）是一个正经的商业词，它把「子代理」整个包住。
  // 这份语料里 0 条，但 0 不等于以后是 0——照 `多 agent` / `TRAE Work` 那两次的规矩，
  // 用 `not` 遮罩挡住，而不是靠「这次没量到」放行。
  { id: 'subagent', name: '子智能体', aka: ['subagent', 'sub agent', '子智能体', '子 agent', '子代理'], not: ['子代理商'] },
  ...
  // Cursor Rules：Cursor 那一侧的「项目记忆」约定（.cursor/rules/ 下的 .mdc 文件，
  // frontmatter 声明什么时候生效）。和 CLAUDE.md / AGENTS.md 是同一族的约定文件，
  // 而词表里那三个都有、唯独缺这一个——2026-09-07 三条掘金稿把它和另外两个
  // 并列讲了一遍，工具挂上了 cursor，约定文件这一维一个标签都没挂上。
  // 别名过三问：`cursor rules` 两个词连着出现只可能指它；它不是别人的名字；
  // 没有别的名字把它整个包住。**裸 `.mdc` 不能要**——MDC 是日志领域的
  // Mapped Diagnostic Context，是个独立的常用缩写，和 `amp` 同族。
  // 空格那条照 08-31 的教训两种写法都写（`cursor rules` / `cursorrules`）。
  // 边界：371 条语料里只命中 1 条，**样本量不够**，加它买的是标签不是分
  //（标签错了就永远错了，已发布条目没有第二次打标签的机会）；
  // 09-14 复量一次，真出现噪声就退成 `score: false`。
  { id: 'cursor-rules', name: 'Cursor Rules', aka: ['cursorrules', '.cursorrules', 'cursor/rules'] },
...
/**
 * 哪些**工具**够得上退役（进 `UNQUERYABLE`）。
 *
 * 手册写的是「换掉长期零产出的查询词」，而能执行的开关粒度是工具，
 * 两者不能直接换算——2026-08-24 定死的口径是
 * **「这个工具派生的所有查询都跑够轮次，且合计零入围」**：
 * 同一轮里 `通义灵码 实践` 5 轮 0 入围，而 `通义灵码 踩坑` / `体验` 各出过 1 条，
 * 只看那一条零产出的查询就会把整个工具砍错。
 *
 * 2026-09-07 周更体检把这一问**交给机器**。理由是账：抓取尾部那行
 * 「跑够 5 轮仍零入围的查询词」今天列了 9 条，而这 9 条**四轮体检里被逐个重算了四次
 * （08-11、08-31、09-07 日更、09-07 周更），四次的答案都是「一个都不该退役」**——
 * 因为它印的是查询粒度，而判据是工具粒度，读的人每次都得自己把两者换算一遍。
 * 这正是 LESSONS 那条「一条每天靠记性重新量一遍的规矩不会收敛，
 * 交给机器之后它才会停止被重新讨论」，和把 `readableCorpus()` /
 * `samplingWindows()` 做进抓取尾部是同一笔账。
 *
 * @param {Record<string,{runs?:number,shortlisted?:number,items?:number}>} ledger query-yield.json
 * @param {number} minRuns 每条派生查询至少跑过几轮才算「跑够」
 */
export function retirableTools(ledger, minRuns = 5) {
  const out = [];
  for (const t of TOOLS) {
    if (UNQUERYABLE.has(t.id)) continue;   // 已经退役的没有手术对象
    const qs = TOOL_TEMPLATES.map((tpl) => tpl.replace('%s', t.name));
    const stats = qs.map((q) => ledger[q]);
    if (stats.some((s) => !s || (s.runs ?? 0) < minRuns)) continue;
    const shortlisted = stats.reduce((a, s) => a + (s.shortlisted ?? 0), 0);
    if (shortlisted > 0) continue;
    out.push({
      id: t.id,
      name: t.name,
      queries: qs.length,
      runs: stats.reduce((a, s) => a + (s.runs ?? 0), 0),
      items: stats.reduce((a, s) => a + (s.items ?? 0), 0),
    });
  }
  return out;
}
