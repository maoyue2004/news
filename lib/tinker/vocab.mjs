...
export function queriesForDate(date, perDay = 24) {
  ...
  const start = ((days * rotating) % SEARCH_QUERIES.length + SEARCH_QUERIES.length) % SEARCH_QUERIES.length;
  const slice = [];
  for (let i = 0; i < rotating; i += 1) slice.push(SEARCH_QUERIES[(start + i) % SEARCH_QUERIES.length]);
