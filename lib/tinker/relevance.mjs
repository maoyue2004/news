 ...
 * 薪资数字是最可靠的判据。
 */
const JOB = /(\d+\s*[kK]\s*[-–~]\s*\d+\s*[kK])|(\d+\s*[wW]\s*[-–~]\s*\d+\s*[wW])|(薪资|月薪|年薪|五险一金|双休|base\s|内推|投递简历|远程全职|全职远程|招聘|诚聘|急招|可接|接私活|承接外包|求职|找工作|年经验[｜|])|(招[\s\S]{0,4}?(全栈|前端|后端|客户端|服务端|算法|架构师|工程师|开发者?|运维|测试|产品经理|设计师|实习生?))/;
...
/**
 * 只有明确标成 forum / search 的才受占比约束。
 * 未知 kind 按文章处理——「拿不准就扣」在这里是错的方向：
 * 占比上限是用来压制已知噪声源的，不该顺手压掉一个只是元数据没写全的条目。
 */
const FORUM_KINDS = new Set(['forum', 'search']);
const isForum = (item) => FORUM_KINDS.has(item.kind);
...
export function triage(items, { cap = 60, quota = PER_SOURCE_QUOTA, forumShare = FORUM_SHARE } = {}) {
  ...
  function byQuota(list, limit) {
    ...
    for (const item of overflow.splice(0)) {
      if (taken.length >= limit) { overflow.push(item); continue; }
      taken.push(item);
    }
    return taken;
  ...
  const takenArticles = byQuota(articles, cap);
  // forumShare 是目标占比，换算成「相对文章数能带多少条论坛」：s/(1-s)。
  const budget = Math.max(FORUM_FLOOR, Math.round((takenArticles.length * forumShare) / (1 - forumShare)));
