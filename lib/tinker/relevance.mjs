...
export function triage(items, { cap = 60, quota = PER_SOURCE_QUOTA, forumShare = FORUM_SHARE, thinShare = THIN_SHARE, publishedTitles = new Set() } = {}) {
  ...
  if (thinPassed.length > thinLimit) {
    ...
    for (const item of passed) {
      ...
      rejected.push(slim(item, item.preScore, [...item.preReasons, '抓不到正文的条目已达当日名额，未进入人工评审'], true));
    }
  ...
  for (const item of passed) {
    ...
    let why;
    if (used >= quota * QUOTA_RELAX) why = `单源配额已满（${item.source} 占 ${used} 席），未进入人工评审`;
    else if (isForum(item)) why = '论坛/搜索源已达当日占比上限，未进入人工评审';
    else why = '超出当日入围上限，未进入人工评审';
    rejected.push(slim(item, item.preScore, [...item.preReasons, why], true));
  }
