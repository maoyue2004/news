...
/**
 * 「选型盘点」体：一条一条介绍工具，每条挂同一套固定小标题。
 *
 * 2026-09-06 量到的：掘金一个内容农场把同一篇「想搭建多智能体协作平台，怎么选适配的
 * AI 工具」换三个标题发了三遍（两条同一个作者），三条全部进了当天 33 席的入围名单，
 * **9% 的评审预算花在同一篇软文上**，而它的落点是「顺手去扣子 app 的技能商店搜一下」。
 *
 * 它躲过了现有的每一道闸，一条都不冤：`SELF_PROMO` 找的是冲着读者的招呼语，
 * 这类稿子一句都没有；严格同题那道闸比的是逐字相同的标题，而它每次换一个说法
 * （「协作项目」/「协作平台」/「这些 AI 工具可以参考」）；机械去重那条路 08-02
 * 已经量死了（真重复的相似度反而低于真正不同的文章）。
 *
 * 能认出它的是**版式**：每介绍一个工具就重复一遍「适合谁 / 解决什么问题 / 核心能力 /
 * 使用边界 / 实际场景」。这是 SEO 工具盘点的固定骨架，一个人记录自己踩的坑不会这么写——
 * 判据和 `JOB_JD_HEADINGS`、`MACHINE_REPORT` 同族，都是**字面即判据**、认版式不认语义。
 *
 * 量过（当天 481 条原始条目、314 条可读正文，全文口径）：命中 1 个小标题的 3 条、
 * ≥2 个的也是同样 3 条、≥3 个的还是这 3 条——**说明这套小标题要么整套出现要么一个不出现**，
 * 拿创刊以来的日文件对过，已收录条目 0 条误伤。门槛取 2 是照 JOB_JD 那条的理由：
 * 一篇正经文章顺口写一次「适合谁：」是可能的，不会连「使用边界：」也一起当小标题。
 *
 * 扣 8 分而不是 6：这三条的分数都是 13（标题命中 multi-agent 6 + 泛词 1 +
 * 标题经验词「搭建」3 + 正文经验词 3-4），扣 6 之后仍是 7 分、照样占席位，
 * 也就是说扣 6 等于没扣。8 分刚好把这一类压到入围线（6 分）以下，
 * 而一篇真的动过手、又恰好用了这套小标题的稿子，分数通常不止 13。
 * **出现第一次误伤就把门槛从 2 提到 3，再有就退回 6 分**，不要整条删。
 *
 * ---
 * **2026-09-07 补两处，第一处比第二处贵得多。**
 *
 * 第一处是**小标题和冒号之间的那个空格**。上面那五条写的是 `适合谁[:：]`，
 * 要求冒号紧挨着小标题；而正文是 `extractArticleText` 从 HTML 里抽出来的，
 * 小标题通常包在 `<strong>` 里，去标签之后落到语料里的是「适合谁 ：」。
 * 当天量得清清楚楚：`/适用人群[:：]/` 命中 **0** 条，`/适用人群\s*[:：]/` 命中 **3** 条，
 * 同一份 371 条语料、同一个词。这是 08-31「别名里的空格是字面的」在**正则**上的复发，
 * 只是那次换的是词与词之间的空格，这次换的是词与标点之间的；
 * 而它的症状最坏——**「这个模板不存在」和「这条正则少写了两个字符」给出同一个 0**，
 * 下一轮复盘会照着这个 0 记下「模板换了」。所以五条一律补 `\s*`。
 *
 * 第二处是这类稿子的**第二套版式**。09-06 那三条是「工具盘点」骨架；
 * 09-07 进名单的是「平替横评」骨架——`Cursor平替方案有哪些` / `Cursor平替怎么选`
 * 两条（另有一条没进名单的「个人AI编程工具怎么选」），版式是
 * 「摘要：从价格、…等维度横向评测 → **适用人群：** → **更新日期：** → **维度对比表** →
 * 不同场景的选择建议」。LESSONS 08-07 早就记过这一类「推广稿会脱掉容易认的外衣」，
 * 记的是标签会换，这次换的是**小标题这一套**——同一个内容农场的两条产线。
 * 补上 `适用人群` / `更新日期` / `维度对比` 三条之后重放当天 247 条可读语料：
 * ≥2 命中 **3 条**（就是那三条横评稿，命中数 3 / 3 / 2），恰好 1 命中的 3 条不扣分
 * （一条开发者自荐、一条多 Agent 平台综述、一条插件实战），已收录条目 **0 误伤**。
 * 分值不用动：那两条的裸分是 13 和 10，扣 8 之后是 5 和 2，都在入围线（6）以下——
 * 照「一条扣分规则的分值要按它要挡的那批稿子的裸分定」验过才敢不动。
 * 边界同上：`更新日期` 是三条里最泛的一条（一篇正经的长期维护文也可能写），
 * 但它单独出现不扣分，要和另一条一起才算数；**真出现误伤先删它，再动门槛。**
 */
const LISTICLE_HEADINGS = [
  /适合谁\s*[:：]|適合誰\s*[:：]/,
  /解决什么问题\s*[:：]|解決什麼問題\s*[:：]/,
  /核心能力\s*[:：]/,
  /使用边界\s*[:：]|使用邊界\s*[:：]/,
...
const LISTICLE_MIN_HEADINGS = 2;
const looksLikeToolListicle = (body) =>
  LISTICLE_HEADINGS.filter((re) => re.test(body)).length >= LISTICLE_MIN_HEADINGS;
...
/**
 * 中文占比门槛。它挡的是**英文源**——LESSONS 里 `tidings` 那条写着
 * 「英文源进来一律撞 `MIN_CJK_RATIO`，扫它们只是白花请求」，这就是那道闸。
 *
 * **2026-09-07 周更体检：它同时在硬毙一类中文文章，而且完全不报警。**
 * 起因是拿 1377 条语料回头对已发布条目，53 条能对上的里有 1 条被内容规则毙掉：
 * 09-06 的 **5 分精选**《AI协同 | 从 LangSmith 到 AGENTS.md：一次 Agent 上下文优化实践》，
 * 理由是「中文占比 3%」。查下去它一个字都没错——作者在开头就贴了整份英文的
 * `AGENTS.md`（连行号 `1 2 3 … 52` 都在），而 `excerpt` 只取正文前 2500 字符，
 * 于是那 2500 字符几乎全是英文代码块，标题那 33% 的中文被稀释到 2.7%。
 *
 * 这是 08-20 那条「任何按位置取样的规则，都要先问一句『我取的这一段，
 * 是不是我以为的那一段』」的又一例，而这次的代价比前几例都大：
 * **`MIN_CJK_RATIO` 是硬毙**，一条撞上它的条目连分都不打、`preReasons` 里
 * 只留一句中文占比，看起来和一篇真的英文文章一模一样。
 * 「太长不读 + 先贴整份配置文件」恰恰是这个刊物最想要的那种写法。
 *
 * 改法取最小的一刀：**标题和正文都不像中文才硬毙**。
 * 一个真正的英文源，标题也是英文的（`tidings` 那批就是这样），这一刀拦得住；
 * 而中文标题 + 代码块开头的文章从此只是分数低，不再是「不存在」。
 * 1377 条语料量下来：撞这道闸的 24 条里，**标题本身是中文的有 14 条**，
 * 逐条看完 4 条对口（上面那篇 5 分、iT 邦「手刻一個 MCP server」、
 * V2EX「分析一个画各种图的 SKILL」、V2EX 技术节点那条 GPT-6-Astra 分享），
 * 其余 10 条是 Rollup 笔记 / 几何题 / 云主机测评这类本来就会被分数拦下的。
 */
...
export function scoreItem({ title = '', excerpt = '', tail = '', metrics = null, thread = false } = {}) {
  ...
  // 标题和正文都不像中文才硬毙——正文那一半会被开头的代码块整段稀释，见 MIN_CJK_RATIO 的注释。
  const ratio = cjkRatio(body);
  const titleRatio = cjkRatio(title);
  if (ratio < MIN_CJK_RATIO && titleRatio < MIN_CJK_RATIO) {
    return reject(`中文占比 ${(ratio * 100).toFixed(0)}%（标题 ${(titleRatio * 100).toFixed(0)}%），低于门槛`);
  }
  ...
  if (looksLikeToolListicle(`${excerpt}\n${tail}`)) {
    score -= 8;
    reasons.push('选型盘点体（固定小标题逐个介绍工具）');
  }
