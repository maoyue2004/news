 ...
 * 仍然只扣 6 分不硬毙，理由同上。
 */
const SELF_PROMO = /(求个?\s*[Ss]tar|求\s*star|欢迎试用|欢迎体验|欢迎大家|求交流|求反馈|求建议|开发者自荐|送会员|注册免费|免费体验|欢迎来|敬请期待|已上线|新版本发布|上线啦|来压测|\[分享开源项目\]|\[开源\]|开源一个|开源了一个|重磅开源|核心功能有|主要功能有|功能特性[:：]|在线体验[:：]|立即体验|歡迎試用|歡迎體驗|歡迎大家|求回饋|開發者自薦|送會員|註冊免費|免費體驗|歡迎來|敬請期待|新版本發布|上線啦|開源一個|開源了一個|重磅開源|線上體驗[:：]|立即體驗)/;
 ...
 */
export function triage(items, { cap = 60, quota = PER_SOURCE_QUOTA, forumShare = FORUM_SHARE, thinShare = THIN_SHARE } = {}) {
  let passed = [];
  const rejected = [];
  ...
  if (thinPassed.length > thinLimit) {
    ...
    for (const item of passed) {
      ...
      rejected.push(slim(item, item.preScore, [...item.preReasons, '抓不到正文的条目已达当日名额，未进入人工评审']));
    }
